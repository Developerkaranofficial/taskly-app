...
tasks.addEventListener("click", (e) => {
	...
	const completeBtn = e.target.closest(".done-btn");

	if (completeBtn) {
		const taskElement = completeBtn.closest(".task");
		const id = Number(taskElement).id;
		const selectTask = taskArr.find((task) => task.id === id);
	...
	taskArr.forEach((task) => {
		const taskHTML = ` <div class="task" id=${task.id}>
            <div class="task-1">
              ...
              <p>${task.detail}</p>
 <div class="icons">
          <svg class="done-btn" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" 
              fill="#e3e3e3"> 
                <path 
